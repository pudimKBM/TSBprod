...
class pjAdminBookings extends pjAdmin
{
    ...
    public function pjActionCreate()
    {
        ...
        if ($this->isAdmin() || $this->isEditor() || $this->isTemp()) {
    ...
    public function pjActionDeleteBooking()
    {
        ...
        if ($this->isXHR() && $this->isLoged()) {
            ...
            if (isset($_GET['id']) && (int) $_GET['id'] > 0 && pjBookingModel::factory()->set('id', $_GET['id'])
                ->erase()
                ->getAffectedRows() == 1) {
    ...
    public function pjActionDeleteBookingBulk()
    {
        ...
        if ($this->isXHR() && $this->isLoged()) {
            if (isset($_POST['record']) && ! empty($_POST['record'])) {
                $uiid_arr = pjBookingModel::factory()->whereIn('id', $_POST['record'])
                    ->findAll()
                    ->getDataPair(NULL, 'uuid');
    ...
    public function pjActionExportICal()
    {
        ...
        if ($this->isAdmin()) {
            if (isset($_POST['booking_export'])) {
                $arr = pjBookingModel::factory()->select('t1.*, t2.*')
    ...
    public function pjActionExport()
    {
        ...
        if ($this->isAdmin()) {
            if (isset($_POST['bookings_export'])) {
                $pjBookingModel = pjBookingModel::factory()->select("t1.*, t2.*,
						...
						AES_DECRYPT(t1.cc_code, '" . PJ_SALT . "') AS `cc_code`")->join('pjBookingSlot', 't2.booking_id=t1.id', 'left outer');
                ...
                $arr = $pjBookingModel->orderBy("$column $direction")
                    ->findAll()
                    ->getData();
                if ($_POST['type'] == 'file') {
                    ...
                    $arr = $pjPasswordModel->where("t1.password", $password)
                        ->limit(1)
                        ->findAll()
                        ->getData();
                    if (count($arr) != 1) {
                        $pjPasswordModel->setAttributes(array(
                            'password' => $password
                        ))->insert();
                    }
            ...
            $calendar_arr = pjCalendarModel::factory()->select('t1.*, t2.content AS `title`')
    ...
    public function pjActionExportFeed()
    {
        ...
        if (isset($_GET['p'])) {
            $pjPasswordModel = pjPasswordModel::factory();
            $arr = $pjPasswordModel->where('t1.password', $_GET['p'])
                ->limit(1)
                ->findAll()
                ->getData();
            if (count($arr) != 1) {
        ...
        if ($access == true) {
            ...
            if (! empty($arr)) {
                if ($_GET['format'] == 'xml') {
                    $xml = new pjXML();
                    echo $xml->setEncoding('UTF-8')
                        ->process($arr)
                        ->getData();
                }
                if ($_GET['format'] == 'csv') {
                    $csv = new pjCSV();
                    echo $csv->setHeader(true)
                        ->process($arr)
                        ->getData();
                }
                if ($_GET['format'] == 'ical') {
                    ...
                    $ical = new pjICal();
                    echo $ical->setProdID('Time Slots Booking Calendar')
                        ->setSummary('summary')
                        ->setCName('desc')
                        ->setLocation('location')
                        ->setTimezone(pjUtil::getTimezoneName($this->option_arr['o_timezone']))
    ...
    public function pjActionGetBooking()
    {
        ...
        if ($this->isXHR() && $this->isLoged()) {
            ...
            if ($this->isEditor() || $this->isTemp()) {
                $calendar_id_arr = pjCalendarUserModel::factory()->where("user_id", $this->getUserId())
                    ->findAll()
                    ->getDataPair(null, 'calendar_id');
            ...
            if (isset($_GET['booking_status']) && ! empty($_GET['booking_status']) && in_array($_GET['booking_status'], array(
                'confirmed',
                'pending',
                'cancelled'
            ))) {
            ...
            $data = $pjBookingModel->select(sprintf("t1.*, t2.content as calendar_title, AES_DECRYPT(t1.cc_type, '" . PJ_SALT . "') AS `cc_type`,
								AES_DECRYPT(t1.cc_num, '" . PJ_SALT . "') AS `cc_num`,
								AES_DECRYPT(t1.cc_exp_month, '" . PJ_SALT . "') AS `cc_exp_month`,
								AES_DECRYPT(t1.cc_exp_year, '" . PJ_SALT . "') AS `cc_exp_year`,
    ...
    public function pjActionGetPrice()
    {
        ...
        if ($this->isXHR() && $this->isLoged()) {
            ...
            if (isset($_POST['id']) && (int) $_POST['id'] > 0) {
                $bs_arr = pjBookingSlotModel::factory()->where('t1.booking_id', $_POST['id'])
                    ->findAll()
                    ->getData();
    ...
    public function pjActionSaveBooking()
    {
        ...
        if ($this->isXHR() && $this->isLoged()) {
            ...
            if (! in_array($_POST['column'], $pjBookingModel->getI18n())) {
                $pjBookingModel->set('id', $_GET['id'])->modify(array(
                    $_POST['column'] => $_POST['value']
                ));
            } else {
                pjMultiLangModel::factory()->updateMultiLang(array(
    ...
    private function __getSchedule($iso_date, $calendar_id)
    {
        ...
        $tmp = $pjBookingSlotModel->select('t1.*, t2.uuid, t2.customer_name, t3.content as calendar_title')
            ->join('pjBooking', "t2.id=t1.booking_id", 'inner')
        ...
        if ($this->isEditor() || $this->isTemp()) {
            $calendars = pjCalendarModel::factory()->select('t1.*, t2.content AS `title`')
                ->join('pjMultiLang', "t2.model='pjCalendar' AND t2.foreign_id=t1.id AND t2.field='title' AND t2.locale='" . $this->getLocaleId() . "'", 'left')
        ...
        if ($this->isXHR() && $this->isLoged()) {
            if (! isset($_POST['pk']) || ! isset($_POST['name']) || ! isset($_POST['value']) || empty($_POST['name']) || (int) $_POST['pk'] <= 0) {
                pjAppController::jsonResponse(array(
                    'status' => 'ERR',
                    'code' => 100,
                    'text' => 'Missing, invalid or empty parameters.'
                ));
            }
            
            $pjBookingModel = pjBookingModel::factory();
            if (! in_array($_POST['name'], $pjBookingModel->getI18n())) {
                if ($pjBookingModel->set('id', $_POST['pk'])
                    ->modify(array(
                    ...
                    pjAppController::jsonResponse(array(
                        'status' => 'ERR',
                        'code' => 100,
                        'text' => 'Sorry, value was not saved.'
                    ));
                }
            } else {
                pjMultiLangModel::factory()->updateMultiLang(array(
    ...
    public function pjActionUpdate()
    {
        ...
        if (isset($_POST['booking_update'])) {
            ...
            $_arr = $pjInvoiceModel->where('t1.order_id', $arr['uuid'])
                ->limit(1)
                ->findAll()
                ->getData();
            $_arr = $_arr[0];
            ...
            $this->set('arr', $arr)->set('country_arr', pjCountryModel::factory()->select('t1.*, t2.content AS `name`')
                ->join('pjMultiLang', "t2.model='pjCountry' AND t2.foreign_id=t1.id AND t2.field='name' AND t2.locale='" . $this->getLocaleId() . "'", 'left outer')
                ->orderBy('`name` ASC')
                ->findAll()
                ->getData());
            
            $this->set('bi_arr', pjBookingSlotModel::factory()->where('t1.booking_id', $arr['id'])
                ->findAll()
                ->getData());
            ...
            if (isset($_POST['item_add'])) {
                if (isset($_POST['timeslot']) && ! empty($_POST['timeslot'])) {
                    ...
                    if (isset($_POST['booking_id']) && (int) $_POST['booking_id'] > 0) {
                        ...
                        if ($duplicated == false) {
                            ...
                            pjAppController::jsonResponse(array(
                                'status' => 'ERR',
                                'code' => 101,
                                'text' => __('lblDuplidatedSlot', true)
                            ));
                        }
                    ...
                    pjAppController::jsonResponse(array(
                        'status' => 'OK',
                        'code' => 200,
                        'text' => 'Booking slot(s) has been added.'
                    ));
                }
                pjAppController::jsonResponse(array(
                    'status' => 'ERR',
                    'code' => 100,
                    'text' => 'Time slots couldn\'t be empty.'
                ));
            }
        }
    }
    ...
    public function pjActionItemDelete()
    {
        ...
        if ($this->isXHR() && $this->isLoged()) {
            if (isset($_POST['id']) && (int) $_POST['id'] > 0) {
                ...
                if (1 == $pjBookingSlotModel->set('id', $_POST['id'])
                    ->erase()
                    ->getAffectedRows()) {
                    pjAppController::jsonResponse(array(
                        'status' => 'OK',
                ...
                pjAppController::jsonResponse(array(
                    'status' => 'ERR',
                    'code' => 102,
                    'text' => 'Slot has not been deleted.'
                ));
            } elseif (isset($_POST['hash']) && ! empty($_POST['hash']) && isset($_POST['key']) && ! empty($_POST['key'])) {
                if (isset($_SESSION[$this->adminBooking]) && isset($_SESSION[$this->adminBooking][$_POST['hash']]) && isset($_SESSION[$this->adminBooking][$_POST['hash']][$_POST['key']])) {
    ...
    public function pjActionItemGet()
    {
        ...
        if ($this->isXHR() && $this->isLoged()) {
            if (isset($_GET['id']) && (int) $_GET['id'] > 0) {
                $bi_arr = pjBookingSlotModel::factory()->select("t1.*")
                    ->join('pjBooking', 't2.id=t1.booking_id', 'inner')
    ...
    public function pjActionReminderEmail()
    {
        ...
        if ($this->isXHR() && $this->isLoged()) {
            if (isset($_POST['send_email']) && isset($_POST['to']) && ! empty($_POST['to']) && ! empty($_POST['from']) && ! empty($_POST['subject']) && ! empty($_POST['message']) && ! empty($_POST['id'])) {
                ...
                $user_arr = pjUserModel::factory()->where('t1.status', 'T')
                    ->where("(t1.notify_email LIKE '%reminder%')")
                    ->findAll()
                    ->getDataPair('id', 'email');
                
                $recipient_arr = array_merge($_POST['to'], $user_arr);
                foreach ($recipient_arr as $recipient) {
                    $r = $Email->setTo($recipient)
                        ->setFrom($_POST['from'])
                ...
                if (isset($r) && $r) {
                    pjBookingModel::factory()->set('id', $_POST['id'])->modify(array(
                        'reminder_email' => 1
                    ));
                    pjAppController::jsonResponse(array(
                        'status' => 'OK',
            ...
            if (isset($_GET['id']) && (int) $_GET['id'] > 0) {
                $booking_arr = pjBookingModel::factory()->select('t1.*, t1.id AS `booking_id`, t2.content AS `country_name`,
                ...
                if (! empty($booking_arr)) {
                    $booking_arr['bs_arr'] = pjBookingSlotModel::factory()->where('t1.booking_id', $_GET['id'])
                        ->findAll()
                        ->getData();
    ...
    public function pjActionReminderSms()
    {
        ...
        if ($this->isXHR() && $this->isLoged()) {
            ...
            if (isset($_GET['id']) && (int) $_GET['id'] > 0) {
                $booking_arr = pjBookingModel::factory()->select('t1.*, t3.email AS `admin_email`, t4.content AS `country_name`, t5.content AS `reminder_sms_client`, t6.content AS `calendar_name`')
